...
public class ConnectionActivity extends AppCompatActivity {
    ...
    public void SignUp(View view) {
        ...
        if (user.length() < 4) {
            Toast.makeText(this, "Username must be 4 or more digits", Toast.LENGTH_SHORT).show();
            return;
        }
        String password = passwordText.getText().toString();
        // checking if the password is okay
        if (password.length() < 6) {
            Toast.makeText(this, "Password must be 6 digits or longer", Toast.LENGTH_SHORT).show();
            return;
        }
